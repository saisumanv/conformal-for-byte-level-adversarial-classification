...
class Attack(object):
    ...
    def get_logits(self, inputs, labels=None, *args, **kwargs):
        if self._normalization_applied is True:#False:#sai
    ...
    def inverse_normalize(self, inputs):
        mean = self.normalization_used["mean"]#.to(inputs.device)#sai
        std = self.normalization_used["std"]#.to(inputs.device)#sai
    ...
    def set_model_training_mode(
        self, model_training=True, batchnorm_training=False, dropout_training=False
    ):
        r"""
    ...
    @wrapper_method
    def _change_model_mode(self):#, given_training):#sai
        if self._model_training:
            self.train()#sai- self.model.train() --> self.train()
            ...
            self.eval()#sai- self.model.eval() --> self.eval()
    ...
    def __call__(self, inputs, labels=None, *args, **kwargs):
        #given_training = self.model.training
        given_training = True #sai
        self._change_model_mode()#given_training)#sai
        # print(type(inputs))
        # print(inputs[0].shape)
        #inputs = torch.as_tensor(inputs)#sai
        #inputs = [torch.from_numpy(item) for item in inputs]#sai
        #inputs = [torch.tensor(item, requires_grad=True) for item in inputs]#sai
        inputs = [torch.tensor(item) for item in inputs]#sai
        ...
        if self._normalization_applied is True:
            ...
            adv_inputs = self.forward(inputs, labels, *args, **kwargs)#sai- function that does the CW or PGD adversarial work
            # adv_inputs = self.to_type(adv_inputs, self.return_type)
        #adv_inputs = self.forward(inputs, labels, *args, **kwargs)
